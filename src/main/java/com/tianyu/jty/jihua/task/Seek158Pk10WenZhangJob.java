 ...
 */
public class Seek158Pk10WenZhangJob implements Job {

    private final static Logger logger = LoggerFactory.getLogger(Seek158Pk10WenZhangJob.class);

    @Override
    public void execute(JobExecutionContext context) throws JobExecutionException {
        ScheduleJob scheduleJob = (ScheduleJob) context.getMergedJobDataMap().get("scheduleJob");
        logger.debug("任务名称 = [" + scheduleJob.getName() + "]" + " pk10在 " + "[开始]--进行文章抓取");
        ...
        wenZhangService.seek158(LotteryTypeEnum.PK10);
        logger.debug("任务名称 = [" + scheduleJob.getName() + "]" + " pk10在 " + "[结束]--进行文章抓取");
    }
}