 ...
 */
public class Seek158Jsk3WenZhangJob implements Job {

    private final static Logger logger = LoggerFactory.getLogger(Seek158Jsk3WenZhangJob.class);

    @Override
    public void execute(JobExecutionContext context) throws JobExecutionException {
        ScheduleJob scheduleJob = (ScheduleJob) context.getMergedJobDataMap().get("scheduleJob");
        logger.debug("任务名称 = [" + scheduleJob.getName() + "]" + " 江苏快3在 " + "[开始]--进行文章抓取");
        ...
        wenZhangService.seek158(LotteryTypeEnum.JSK3);
        logger.debug("任务名称 = [" + scheduleJob.getName() + "]" + " 江苏快3在 " + "[结束]--进行文章抓取");
    }
}
